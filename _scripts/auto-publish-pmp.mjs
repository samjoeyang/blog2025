...
## ⚠️ Google 高质量内容标准（必须遵守）

### 1. E-E-A-T 信号（Experience. Expertise. Authoritativeness. Trustworthiness）
- 必须以第一人称（"我"）讲述一个**真实的、具体的个人经历**作为开篇
- 经历必须包含具体细节：时间、场景、人物的反应、决策的逻辑
- 不能使用"我有个朋友""我见过一个案例"等模糊第三人称；要说"之前我管XX项目的时候"
- 展示专业判断和经验，不能像教科书或百科

### 2. 禁止 AI 文风（以下情况必须避免）
- ❌ 不要用"首先、其次、最后""第一、第二、第三"等模板式过渡词
- ❌ 不要用"核心结论""简单来说""归根结底""综上所述"等AI高频套话
- ❌ 不要连续使用——破折号——分段（整篇文章最多2处）
- ❌ 不要每个段落都结构工整（字数、句式要有变化）
- ❌ 不要使用表格（表格是明显的AI模板特征）

### 3. 个人经验真实性
- 每个论点背后必须有真实经历支撑，而不是"我们都知道""一般来说"
- 具体细节比抽象道理更重要：说"那个项目延期了3周，因为供应商换了3次对接人"而不是"项目可能会遇到延期问题"
- 可以在文中提及"我"的困惑、误判、学到的教训——暴露弱点比显得完美更可信

### 4. 对读者有价值的原创内容
- 提供独特视角或鲜为人知的操作细节，而不是网上随处可见的常识
- 每篇文章至少包含**一个你亲手做的事情/方法/工具**，并解释为什么而做
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!content.includes('我')) {
        console.warn(`  ⚠️ 缺少第一人称「我」，可能不符合EEAT标准，重试...`);
        continue;
      }
      // 2. 不能使用模糊第三人称开头
      const fuzzyPatterns = ['我有个朋友', '我见过一个案例', '我听说过', '有一个项目经理', '有一个同事'];
      const hasFuzzyStart = fuzzyPatterns.some(p => {
        const idx = content.indexOf(p);
        if (idx < 0) return false;
        return idx < 200;
      });
      if (hasFuzzyStart) {
        console.warn(`  ⚠️ 使用了模糊第三人称开头，可能不符合EEAT标准，重试...`);
        continue;
      }
